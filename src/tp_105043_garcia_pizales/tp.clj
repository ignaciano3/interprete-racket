(ns tp-105043-garcia-pizales.tp
  (:require [clojure.string :as st :refer [blank? starts-with? ends-with? lower-case]]
            ...
            [tp-105043-garcia-pizales.spy :refer [spy]]))
...
; Funciones auxiliares de las funciones auxiliares}
(declare buscar-aux)
(declare actualizar-amb-aux)
...
(defn leer-entrada
  ...
  ([subcadena]
   (let [nuevacadena (read-line)]
     (if (nil? nuevacadena)
       subcadena
       (let [cadena (str subcadena " " nuevacadena)
             cant (verificar-parentesis cadena)]
         (cond
           (neg? cant) (str (generar-mensaje-error :warning-paren) cadena)
           (= cant 0) cadena
           :else (leer-entrada cadena))))))
  ([]
   (let [cadena (read-line)]
     (if (nil? cadena)
       ""
       (let [cant (verificar-parentesis cadena)]
         (cond
           (neg? cant) (str (generar-mensaje-error :warning-paren) cadena)
           (= cant 0) cadena
           :else (leer-entrada cadena)))))))
...
; user=> (verificar-parentesis "(hola '(mundo) )")
; 0
(defn verificar-parentesis-aux
  [cadena, cant]
  (cond
    (neg? cant) -1 
    (empty? cadena) cant
    (= (first cadena) \() (verificar-parentesis-aux (rest cadena) (+ cant 1))
    (= (first cadena) \)) (verificar-parentesis-aux (rest cadena) (- cant 1))
    :else (verificar-parentesis-aux (rest cadena) cant)))
...
(defn verificar-parentesis
  ...
  [cadena]
  (verificar-parentesis-aux cadena 0))
...
(defn actualizar-amb-aux
  "Devuelve la posicion de un elemento en un ambiente.
   Si no existe, devuelve el largo del ambiente."
  [amb, clave]
  (cond
    (empty? amb) 0
    (= (first amb) clave) 0
    :else (+ 1 (actualizar-amb-aux (rest amb) clave))))
...
(defn actualizar-amb
  ...
  (if (error? valor)
    amb
    (let [pos (actualizar-amb-aux amb clave)]
        (concat (take pos amb) (list clave valor) (drop (+ pos 2) amb)))))
...
(defn buscar-aux
  [clave, amb, pos]
  (cond
    (empty? amb) (generar-mensaje-error :unbound-variable clave)
    (= (first amb) clave) (second amb)
    :else (buscar-aux clave (rest amb) (+ pos 2))))

(defn buscar
  "Busca una clave en un ambiente (una lista con claves en las posiciones impares [1, 3, 5...] y valores en las pares [2, 4, 6...]
   y devuelve el valor asociado. Devuelve un error :unbound-variable si no la encuentra."
  [clave, amb]
  (buscar-aux clave amb 1))
...
(defn error?
  ...
  [arg]
  (or (and (seq? arg) (= (first arg) (symbol ";ERROR:")))
      (and (seq? arg) (= (first arg) (symbol ";WARNING:")))))
...
(defn fnc-append
  ...
  (cond
    (empty? lista) ()
    ((comp not seq?) (first lista)) (generar-mensaje-error :only-proper-lists-implemented 'append)
    (reduce And (map seq? lista)) (reduce concat lista)
    :else (generar-mensaje-error :wrong-type-arg 'append (first (filter (comp not seq?) lista)))))
...
(defn fnc-equal?
  ...
  [lae]
  (let [ari (controlar-aridad-fnc lae 2 'equal?)]
       (if (error? ari)
           ari
           (if (= (first lae) (second lae))
             (symbol "#t")
             (symbol "#f")))))
...
; (;ERROR: +: Wrong type in arg2 A)
(defn check-non-number
  "Encuentra el primer elemento de una lista que no es un numero."
  [lista]
  (cond
    (empty? lista) nil
    (not (number? (first lista))) (first lista)
    :else (check-non-number (rest lista))))

(defn fnc-sumar
  "Suma los elementos de una lista."
  [lista]
  (let [non-number (check-non-number lista)]
    (if (nil? non-number)
      (apply + lista)
...
(defn fnc-restar
  ...
  [args] 
  (if (empty? args) (generar-mensaje-error :wrong-number-args 'restar)
    (let [non-number (check-non-number args)]
         (cond 
           (= (count args) 1) (- (first args))
           (nil? non-number) (apply - args)
           :else (generar-mensaje-error :wrong-type-arg '- non-number)))))
...
(defn fnc-menor
  ...
  [lista]
  (let [non-number (check-non-number lista)]
    (if (nil? non-number) (if (apply < lista) (symbol "#t") (symbol "#f"))
     (generar-mensaje-error :wrong-type-arg '< non-number))))
...
(defn evaluar-escalar
  ...
  [escalar, amb]
  (cond
    (= escalar (symbol "#t")) (list (symbol "#t") amb)
...
(defn evaluar-define-lambda
  [define, nombre, variables, body, amb]
  (cond
    (not (symbol? nombre)) (list (generar-mensaje-error :bad-variable define) amb)
    (not (seq? variables)) (list (generar-mensaje-error :bad-params define) amb)
    :else (list (symbol "#<void>") (actualizar-amb amb nombre (concat (list 'lambda variables) body)))))
...
(defn evaluar-if
  ...
  [expre, amb]
  (cond
    (not= (count expre) 4) (list (generar-mensaje-error :missing-or-extra 'if expre) amb)
    :else (let [res (evaluar (second expre) amb)]
            (if (error? (first res))
              res 
              (if (= (first res) (symbol "#f"))
                (evaluar (last expre) amb)
                (evaluar (nth expre 2) amb))))))
...
(defn evaluar-or-aux
  [expre, amb]
   (let [res (evaluar (first expre) amb)]
     (if (error? (first res))
       res
       (cond
         (= (first res) (symbol "#t")) (list (symbol "#t") amb)
         (= (count expre) 1) (list (first res) amb)
          :else (evaluar-or-aux (rest expre) amb)))))
...
(defn evaluar-set!
  ...
  [lista, amb]
  (cond
    (not= (count lista) 3) (list (generar-mensaje-error :missing-or-extra 'set! lista) amb)
    (not (symbol? (second lista))) (list (generar-mensaje-error :bad-variable 'set! lista) amb)
    (error? (buscar (second lista) amb)) (list (generar-mensaje-error :unbound-variable (second lista)) amb)
    :else (let [res (evaluar (last lista) amb)]
            (if (error? (first res))
              res
              (list (symbol "#<void>") (actualizar-amb amb (second lista) (first res) ))))))
